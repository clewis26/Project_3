...
model.add(Convolution2D(32, 10, 10,init='he_normal', border_mode='same', input_shape=(1,60, 60)))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))


model.add(Convolution2D(64, 5, 5,init='he_normal', border_mode='same'))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))

model.add(Convolution2D(128, 3, 3,init='he_normal', border_mode='same'))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
...
model.fit(data_in, data_out, nb_epoch=30, batch_size=50, verbose=1, validation_split=0.1)

classes = model.predict_classes(data_in_tst, batch_size=50)

my_df = pd.DataFrame(classes)
my_df.columns=['Prediction']
my_df.to_csv(r'test_out3.csv', index_label="Id")
